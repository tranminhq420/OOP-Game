...
public class Hero extends GameObjectDynamic {
	...
	public void move() {
		...
		if (x > Board.getSizeX() - this.width) {
			x = Board.getSizeX() - this.width;
		}
		if (y > Board.getSizeY() - this.height) {
			y = Board.getSizeY() - this.height;
		}
		// Khong cho di xuyen tuong
	...
	public void keyPressed(KeyEvent e) {
		...
		if (key == KeyEvent.VK_LEFT) {
			dx = -1;
			loadImage("res/textures/img/left.png");
			setDirect(-1);
		}
		if (key == KeyEvent.VK_RIGHT) {
			dx = 1;
			loadImage("res/textures/img/right.png");
			setDirect(1);
		}
		if (key == KeyEvent.VK_UP) {
			dy = -1;
			loadImage("res/textures/img/up.png");
			setDirect(2);
		}
