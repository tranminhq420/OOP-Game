...
public class Boss extends Monster implements Boss_interface {
	...
	public void move(int heroY) { // hàm move() có tham số >< move() kế thừa từ Monster
		if (getDirect() == 1) {
			x += MONSTER_SPEED;
			loadImage("res/textures/img/bossright.png");
		} else if (getDirect() == -1) {
			x -= MONSTER_SPEED;
			loadImage("res/textures/img/bossleft.png");
		} else if (getDirect() == 2) {
			y -= MONSTER_SPEED;
			loadImage("res/textures/img/bossup.png");
		} else if (getDirect() == -2) {
			y += MONSTER_SPEED;
			loadImage("res/textures/img/bossdown.png");
		}
		...
		if (timez == 100) { // cứ sau 100 chu kỳ timer.DELAY lại chuyển hướng di chuyển
			setDirect(rd.nextInt(5) - 2); // random hướng di chuyển (0..5 -2 --> -2 ..2 hướng di chuyển đã quy định 0
		...
		if (x > Board.getSizeX() - 150 - this.width * 2) {
			x = Board.getSizeX() - 150 - this.width * 2;
		}
		if (y > Board.getSizeY() - this.height * 2) {
			y = Board.getSizeY() - this.height * 2;
		}
